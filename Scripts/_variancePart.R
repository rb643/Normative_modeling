...
varianceStats <- function(measure, parcellation, threshold, ...) {
#measure <- "MeanCurv"
#parcellation <- "HCP"
  basedir <- paste("./Output_",parcellation,"/",measure,"_Age_IQ_Match/",sep="")

  if (threshold == TRUE){
    vdir <- paste(basedir,"Variance_Threshold/",sep="")
...
form <- ~ AGE_AT_SCAN + (1|DX_GROUP) + FIQ + (1|SEX) + SRS_TOTAL_T + VIQ + HANDEDNESS_SCORES + (1|SITE_ID)
